...
# Латинские буквы, неотличимые на вид от кириллических. В названиях они
# попадаются вперемешку: «Шампунь SPLASH для волос c кондиционером» написан
# с ЛАТИНСКОЙ «c», а в налоговой базе тот же товар — с кириллической. На
# глаз одно и то же, для сравнения строк — разные товары.
_ОДНОЛИКИЕ = str.maketrans("acepxykmtbh", "асерхукмтвн")


def _norm_product(s: str | None) -> str:
    """Ключ сопоставления номенклатуры между контурами и документами.

    Названия пишутся по-разному: закупка «Подгузники StarKid размер L*4»,
    продажа «Детские подгузники StarKid размер L», а в налоговой базе тот же
    шампунь называется без марки — «Шампунь для жирных волос» против
    «Шампунь SPLASH для жирных волос». Убираем фасовку «*N», слова, которые
    есть только в одном контуре, пунктуацию, регистр и разнобой латиницы с
    кириллицей — остальное должно совпасть."""
    s = (s or "").lower().replace("ё", "е")
    s = re.sub(r"\*\s*\d+", " ", s)
    # Марку убираем ДО замены однолики́х: после неё «splash» превратится в
    # «sрlаsн» с кириллическими буквами и по имени уже не найдётся.
...
def _product_keys(db: Session):
    ...
    def key_of(product: str | None, guid: str | None = None) -> str:
        ...
        # GUID становится ключом, только если справочник его знает. Незнакомый
        # GUID пришлось бы держать ключом самому по себе, а строки без GUID
        # свести к нему нечем — и одна позиция разъезжается на две: движения
        # свежей выгрузки отдельно, вся прежняя история отдельно. Мост через
        # справочник — единственное, что удерживает вместе выгрузки разной
        # свежести, и без него имя надёжнее ключа, которого никто не знает.
