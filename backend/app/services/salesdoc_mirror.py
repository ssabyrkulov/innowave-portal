...
def agent_by_client_1c(db: Session) -> dict:
    ...
    По наблюдению, в выдаче getOrder заказы деактивированных агентов не
    встречаются — но это корреляция, а не доказанное правило API (уволенный
    агент мог просто перестать работать). Полагаться на неё как на факт
    нельзя; признак active всё равно
