...
RULES = {
    ...
    "numbering_gap": {
        ...
        "severity": "warning",
        "hint": "Между соседними номерами есть дыра: номер документу 1С "
                "присвоила, а в выгрузку он не попал. Обычно это непроведённый "
                "документ или помеченный на удаление — пока 1С не отдаёт эти "
                "признаки колонками, дыра в нумерации единственный способ их "
                "увидеть. Проверяется по реализациям, закупкам и списаниям.",
    },
    "price_anomaly": {
...
def run_checks(db: Session, date_from: date | None = None,
    ...
    #
    # Это единственный способ увидеть непроведённые и удалённые документы,
    # пока 1С не отдаёт признаки «Проведен» и «ПометкаУдаления»: номер такому
    # документу присвоен, а в выгрузку он не попал — остаётся дыра. Поэтому
    # проверяем не только реализации, но и закупки со списаниями: там дыра
    # значит ровно то же самое, а последствия те же — товарный баланс.
    doc_sources = [
        ("реализации", [s.doc_number for s in sales if s.doc_number]),
        ("закупки", [r.doc_number for r in db.query(models.Purchase.doc_number)
                     .filter(models.Purchase.doc_number.isnot(None)).all()]),
        ("списания", [r.doc_number for r in db.query(models.WriteOff.doc_number)
                      .filter(models.WriteOff.doc_number.isnot(None)).all()]),
    ]
    for label, numbers in doc_sources:
        by_prefix: dict[str, list] = defaultdict(list)
        for num in set(numbers):
            # Префикс — всё до последней группы цифр. Шаблон «\D*\d+» не
            # подходил под формат 1С «0000-000053»: он требует, чтобы до
            # цифр не было цифр, а там префикс информационной базы. Правило
        ...
        for prefix, nums in by_prefix.items():
            ...
            for a, b in zip(seq, seq[1:]):
                ...
                if 0 < gap <= 20:  # огромные дыры = другой диапазон, не сигналим
                    violations.append(_v(
                        "numbering_gap", f"{label}|{prefix}{a}-{b}",
                        f"{prefix}…", None, None,
                        f"{label}: между №{a} и №{b} отсутствует документов: {gap}",
                    ))
