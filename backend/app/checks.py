...
def run_checks(db: Session, date_from: date | None = None,
    ...
    # Пропуски в нумерации больше не проверяем. Правило появилось, когда 1С
    # не отдавала признаки «Проведён» и «ПометкаУдаления», и дыра в номерах
    # была единственным способом их увидеть. Теперь эти признаки приходят
    # колонками, а непроведённое и помеченное на удаление показывает отчёт
    # «Проблемные документы 1С» — прямо, с номерами и суммами. Сам по себе
    # пропуск номера ошибкой не является: бухгалтер завёл документ и удалил,
    # 1С номер не переиспользует — и сигналить тут не о чем.
