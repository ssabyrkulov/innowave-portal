...
## Третья фирма: Blue Carbon

В той же папке лежат выгрузки ОсОО «Блю Карбон Трейдинг Компани» —
отдельного бизнеса (солнечные панели, инверторы, батареи), который ведётся
в той же 1С той же обработкой. Файлы называются `БлюКарбон_Тип.xlsx` без
слова о контуре: база одна, это управленка. Портал узнаёт фирму по первому
слову имени (`БлюКарбон`, `Blue Carbon`, `BlueCarbon`) и кладёт данные под
`org = bluecarbon`. В переключателе фирм это отдельный пункт «Blue Carbon»;
в «Обе фирмы» (группа Innowave) он не входит — чужой товар и чужие клиенты
в сводных остатках подгузников ни к чему.

Почему это важно: до появления третьей фирмы незнакомое имя падало в
запасную фирму папки — Хайджин. Журнал проводок и остатки грузятся
заменой по фирме, так что чужой пакет стёр бы журнал Хайджина и подменил
его остатки. Файл без узнаваемой фирмы в имени по-прежнему уходит в
запасную фирму папки, поэтому новую базу надо сначала завести в портале
(`ORGS` в `models.py`, `org_from_name` в `integrations.py`), а потом класть
её файлы в папку.
