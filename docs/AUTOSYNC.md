...
const FOLDERS = [
  // Новый формат: одна папка на все четыре базы. Фирму и контур портал
  // берёт из имени файла («Хайджин_НАЛОГОВАЯ_Реализация товары»), поэтому
  // org и ledger здесь — только запасной вариант для файлов без имени фирмы.
  { id: '1m8Ho_qeLuLJJg2cI1XdrJ2HDRJ6dUtAo', org: 'hygiene' },                  // INNOWAVE GROUP UPLOAD
  // Старые папки. Отключаются (просто закомментируйте строку) после того,
  // как контрольные суммы по новой папке сойдутся — см. ПЛАН_ПЕРЕХОДА.
  // Налоговую папку отключите СРАЗУ вместе с подключением новой: старые
  // налоговые файлы портал уже не принимает, а лежать двумя снапшотами
  // рядом им нельзя.
  ...
  // { id: '1Xtjseug4Pp863O1b307W4DbB_8QmCX6x', org: 'hygiene', ledger: 'nal' }, // Налоговая база (ред. 1.7) — отключена
];
