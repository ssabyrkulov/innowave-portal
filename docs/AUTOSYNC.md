...
Колонка `ДокументGUID` в выгрузках поступлений и списаний — тот же
идентификатор, который SalesDoc отдаёт в поле `code_1C`. Он сохраняется и
делает связку документов точной: без него совпадение искалось по сумме и дате
с допуском, и любая правка документа его рвала. Если такую колонку добавить и
в выгрузку реализаций, сверка с SalesDoc перестанет зависеть от догадок.
