...
const VERDICTS = {
  ...
  guess: { icon: '≈', label: 'сошлось без ИД', cls: '',
           hint: 'Идентификатора нет; совпали контрагент (или номенклатура), сумма либо количество и дата в пределах трёх дней — вероятно, одна и та же операция' },
  ...
  return (
    <div>
      ...
      <div className="note-readonly">
        ...
        это догадка, а не факт. Списания и поступления сверяются
        <b>построчно</b>: в 1С это один документ на несколько позиций, а в
        SalesDoc — отдельный документ на каждую, и документы не совпали бы
        никогда, тогда как строки совпадают точно.
      </div>
      ...
      {data && !data.has_sd && (
        <div className="note-readonly sd-warn">
          Сторона SalesDoc здесь не подключена — сверка не проводилась.
        </div>
      )}
      {data && data.has_1c === false && (
        <div className="note-readonly sd-warn">
          ...
          было.
          {data.sd_with_guid === 0 && data.sd_docs > 0 && (
            <> Но и после появления выгрузки сверка по идентификатору здесь не
            заработает: <code>code_1C</code> пуст у всех {data.sd_docs}{' '}
            документов SalesDoc — связывать придётся по дате, складам и
            количеству.</>
          )}
        </div>
      )}
        ...
        <table>
          <thead>
            <tr>
              <th>Метка</th>
              <th>Дата</th>
              <th>{byQty ? 'Номенклатура' : 'Контрагент'}</th>
              <th>1С</th>
